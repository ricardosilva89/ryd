...
export function deletePoiByIdRoute(router: Router) {
	router.delete('/poi/:id', async (context: Koa.Context) => {
		...
		await poiManager.remove(context.params.id);

		return (context.body = true);
	});
}
